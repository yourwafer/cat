...
public class RouterConfigManager implements Initializable, LogEnabled {
    ...
    private Map<String, List<OperatorServerRange>> m_subNetInfos = new HashMap<String, List<OperatorServerRange>>();
    ...
    private String queryGroupBySubnet(String ip) {
        for (Entry<String, List<OperatorServerRange>> entry : m_subNetInfos.entrySet()) {
            List<OperatorServerRange> subnetInfos = entry.getValue();
            String group = entry.getKey();

            for (OperatorServerRange info : subnetInfos) {
                try {
    ...
    private void refreshNetInfo() {
        Map<String, List<OperatorServerRange>> subNetInfos = new HashMap<String, List<OperatorServerRange>>();

        for (Entry<String, NetworkPolicy> netPolicy : m_routerConfig.getNetworkPolicies().entrySet()) {
            ArrayList<OperatorServerRange> infos = new ArrayList<OperatorServerRange>();

            if (!DEFAULT.equals(netPolicy.getKey())) {
                for (Entry<String, Network> network : netPolicy.getValue().getNetworks().entrySet()) {
                    try {
                        String ipConfig = network.getValue().getId();
                        String[] split = ipConfig.split("\\.");
                        int minOid = Integer.parseInt(split[0]);
                        int maxOid = Integer.parseInt(split[1]);
                        if (minOid > maxOid) {
                            continue;
                        }
                        int minSid = Integer.parseInt(split[2]);
                        int maxSid = Integer.parseInt(split[3]);
                        if (minSid > maxSid) {
                            continue;
                        }
                        infos.add(new OperatorServerRange(minOid, maxOid, minSid, maxSid));
                    } catch (Exception e) {
                        Cat.logError(e);
                    }
                }
